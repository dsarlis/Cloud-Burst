...
public class Q6Servlet extends HttpServlet {
    ...
    /* method to find out the number of your server instance so
     * the server knows what keys it needs to process and
     * what keys it needs to redirect to other server instances
     */
    ...
    /* Function to wait until it is your turn to handle the request
     * Essentially, we use an AtomicInteger lock to order
     * the requests for the same transcation id by their sequence id
     * So seq=2 will wait for seq=1, seq=3 will wait for seq=1 and seq=2
